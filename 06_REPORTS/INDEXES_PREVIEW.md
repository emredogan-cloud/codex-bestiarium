# Dizin önizlemesi — Codex Bestiarium

<!-- OTOMATİK ÜRETİLDİ — 08_BUILD/make_index.py · ELLE DÜZENLEMEYİN -->

Dört dizin `01_SOURCE/spec.json`'dan üretilir. Sayfa numaraları dizgiden
okunur; `—` görüyorsanız o sürüm henüz dizilmemiştir.

## 1. Gelenek dizini

| Gelenek | İşaret | Bölge | Cilt 1 | Maddeler |
|---|---|---|---|---|
| Ainu | ᚼ | Doğu Asya | — | Kenas-unarpe (—) · Koropokkuru (—) · Repun Kamuy (—) |
| Anishinaabe | ▲ | Kuzey Amerika | — | Animikii (—) · Mishipeshu (—) · Windigo (—) |
| Bharatiya | ॐ | Güney Asya | ✓ | Garuḍa (—) · Makara (—) · Nāga (—) |
| Bod | ☷ | Himalaya | — | Khyung (—) · Migoi (—) · Srin-po (—) |
| Dacia | ✠ | Balkanlar | — | Strigoi (—) · Vârcolac (—) · Zmeu (—) |
| Ériu | ☘ | Kuzey Avrupa | ✓ | Bean sídhe (—) · Each-uisce (—) · Púca (—) |
| Euskal | ✜ | Batı Avrupa | — | Basajaun (—) · Herensuge (—) · Lamia (—) |
| Filipin | ✧ | Güneydoğu Asya | — | Aswang (—) · Manananggal (—) · Tikbalang (—) |
| Hangug | 단 | Doğu Asya | ✓ | Dokkaebi (—) · Imugi (—) · Kumiho (—) |
| Hayk | ✚ | Kafkasya | — | Aralez (—) · Nhang (—) · Vishap (—) |
| Hellenic | Ω | Akdeniz | ✓ | Chímaira (—) · Kérberos (—) · Lámia (—) |
| Inuit | ᐃ | Kutup | ✓ | Amarok (—) · Qalupalik (—) · Tupilaq (—) |
| Ísland | ❆ | Kuzey Avrupa | — | Huldufólk (—) · Nykur (—) · Skuggabaldur (—) |
| Ityop'ya | ✤ | Afrika | — | Buda (—) · Ganen (—) · Zar (—) |
| Kartveli | ✛ | Kafkasya | — | Devi (—) · Kaji (—) · Ochokochi (—) |
| Kemet | 𓂀 | Akdeniz | ✓ | Ammit (—) · Apep (—) · Bennu (—) |
| Mā'ohi | ᴥ | Okyanusya | ✓ | Moʻo (—) · Ponaturi (—) · Taniwha (—) |
| Maya | 𝋠 | Mezoamerika | ✓ | Camazotz (—) · Way (—) · Xtabay (—) |
| Melanesia | ◉ | Okyanusya | — | Adaro (—) · Masalai (—) · Temes Savsap (—) |
| Mēxihcah | ☼ | Mezoamerika | ✓ | Ahuizotl (—) · Cipactli (—) · Nahual (—) |
| Mongol | ⚔ | Orta Asya | — | Almas (—) · Chötgör (—) · Olgoi-Khorkhoi (—) |
| Nguni | ◈ | Afrika | — | Impundulu (—) · Inkanyamba (—) · Tokoloshe (—) |
| Norðr | ᚦ | Kuzey Avrupa | ✓ | Draugr (—) · Fenrir (—) · Jörmungandr (—) |
| Nusantara | ❋ | Güneydoğu Asya | — | Orang Bunian (—) · Pontianak (—) · Rangda (—) |
| Pārs | 𐎩 | Yakın Doğu | ✓ | Aži Dahāka (—) · Perī (—) · Sīmurgh (—) |
| Romana | SPQR | Akdeniz | ✓ | Basiliscus (—) · Lemures (—) · Strix (—) |
| Sápmi | ❄ | Kutup | — | Gufihtar (—) · Stállu (—) · Ulda (—) |
| Siam | ☸ | Güneydoğu Asya | — | Kinnarī (—) · Krasue (—) · Phaya Nak (—) |
| Slovjan | ⚡ | Kuzey Avrupa | ✓ | Domovoy (—) · Rusalka (—) · Zmey (—) |
| Sumer | 𒀭 | Yakın Doğu | ✓ | Anzû (—) · Ḫumbaba (—) · Lamashtu (—) |
| Suomi | ᛉ | Kuzey Avrupa | — | Ajatar (—) · Iku-Turso (—) · Näkki (—) |
| Talmud | ✡ | Yakın Doğu | — | Golem (—) · Lilith (—) · Ziz (—) |
| Tawantinsuyu | ☉ | And | — | Amaru (—) · Pishtaco (—) · Supay (—) |
| Tupi-Guarani | ❂ | Amazon | — | Boitatá (—) · Curupira (—) · Iara (—) |
| Türk | ☾ | Orta Asya | ✓ | Al Karısı (—) · Karakoncolos (—) · Şahmeran (—) |
| Việt | ☴ | Güneydoğu Asya | — | Ma lai (—) · Ông Ba Mươi (—) · Thuồng luồng (—) |
| Yamato | 神 | Doğu Asya | ✓ | Kappa (—) · Tengu (—) · Yamata-no-Orochi (—) |
| Yorùbá · Ashanti | ✺ | Afrika | ✓ | Àbíkú (—) · Adze (—) · Anansi (—) |
| Zhōnghuá | 龍 | Doğu Asya | ✓ | Húli jīng (—) · Lóng (—) · Qílín (—) |
| ʿArab | ☪ | Yakın Doğu | ✓ | Ghūl (—) · Rukh (—) · ʿIfrīt (—) |

## 2. Motif dizini

| Motif | Bölüm | Maddeler |
|---|---|---|
| `A177` | A | Anansi (—) |
| `A812` | A | Cipactli (—) |
| `B11` | B | Herensuge (—) · Zmeu (—) · Zmey (—) |
| `B11.1` | B | Boitatá (—) · Imugi (—) |
| `B11.2` | B | Amaru (—) · Aži Dahāka (—) · Iku-Turso (—) · Inkanyamba (—) · Thuồng luồng (—) |
| `B11.2.1.1` | B | Apep (—) · Jörmungandr (—) |
| `B11.2.3` | B | Yamata-no-Orochi (—) |
| `B11.3` | B | Taniwha (—) · Vishap (—) |
| `B11.7` | B | Lóng (—) |
| `B12` | B | Basiliscus (—) |
| `B15` | B | Chímaira (—) |
| `B16` | B | Ahuizotl (—) · Makara (—) · Mishipeshu (—) |
| `B19` | B | Qílín (—) |
| `B31` | B | Anzû (—) · Garuḍa (—) · Khyung (—) · Sīmurgh (—) · Ziz (—) |
| `B31.1` | B | Animikii (—) · Camazotz (—) · Impundulu (—) · Rukh (—) |
| `B32` | B | Bennu (—) |
| `B52` | B | Kinnarī (—) |
| `B91` | B | Moʻo (—) · Nāga (—) · Phaya Nak (—) · Şahmeran (—) |
| `B184.1` | B | Tikbalang (—) |
| `B184.1.3` | B | Each-uisce (—) · Näkki (—) · Nykur (—) |
| `B472` | B | Repun Kamuy (—) |
| `B733` | B | Aralez (—) |
| `B765` | B | Olgoi-Khorkhoi (—) |
| `B871` | B | Amarok (—) · Ông Ba Mươi (—) · Skuggabaldur (—) |
| `B871.1` | B | Fenrir (—) |
| `D110` | D | Nahual (—) · Way (—) |
| `D113` | D | Nhang (—) |
| `D113.1` | D | Húli jīng (—) · Kumiho (—) · Vârcolac (—) |
| `D113.2` | D | Buda (—) |
| `D131` | D | Púca (—) |
| `D150` | D | Tengu (—) |
| `D1635` | D | Golem (—) |
| `D1711` | D | Tupilaq (—) |
| `E200` | E | Chötgör (—) · Lemures (—) · Supay (—) |
| `E220` | E | Àbíkú (—) |
| `E230` | E | Draugr (—) |
| `E251` | E | Strigoi (—) |
| `E402` | E | Bean sídhe (—) |
| `E425` | E | Pontianak (—) |
| `F150` | F | Ḫumbaba (—) · Kérberos (—) · Temes Savsap (—) |
| `F251` | F | Huldufólk (—) · Orang Bunian (—) |
| `F300` | F | Gufihtar (—) · Perī (—) |
| `F302` | F | Xtabay (—) |
| `F400` | F | Dokkaebi (—) · Kaji (—) |
| `F402` | F | Zar (—) · ʿIfrīt (—) |
| `F420` | F | Adaro (—) · Iara (—) · Kappa (—) · Lamia (—) · Rusalka (—) |
| `F451` | F | Koropokkuru (—) · Tokoloshe (—) · Ulda (—) |
| `F460` | F | Almas (—) · Basajaun (—) · Curupira (—) · Masalai (—) · Migoi (—) · Ochokochi (—) |
| `F480` | F | Domovoy (—) |
| `G11` | G | Ghūl (—) · Pishtaco (—) · Windigo (—) |
| `G100` | G | Devi (—) · Stállu (—) |
| `G262` | G | Adze (—) · Aswang (—) · Krasue (—) · Ma lai (—) · Strix (—) |
| `G262.0.1` | G | Kenas-unarpe (—) |
| `G262.1` | G | Manananggal (—) |
| `G263` | G | Rangda (—) |
| `G264` | G | Al Karısı (—) · Lamashtu (—) · Lámia (—) · Lilith (—) |
| `G300` | G | Ganen (—) · Ponaturi (—) · Srin-po (—) |
| `G302` | G | Ajatar (—) · Karakoncolos (—) |
| `G303` | G | Ammit (—) |
| `G308` | G | Qalupalik (—) |

## 3. Akraba imge tablosu

### A · Su atı — *The Water Horse* · `B184.1.3`

> Suyun kıyısında duran, sırtına bineni suya çeken at

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Each-uisce | Ériu ☘ | — |
| Näkki | Suomi ᛉ | — |
| Tikbalang | Filipin ✧ | — |
| Nykur | Ísland ❆ | — |

**Ayrışma noktası.** İrlanda'da yiyicidir, İzlanda'da boğar, Filipinler'de yolu şaşırtır.

### B · Tilki kadın — *The Fox Woman* · `D113.1`

> Kadına dönüşen, ömür veya karaciğer alan tilki

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Húli jīng | Zhōnghuá 龍 | — |
| Kumiho | Hangug 단 | — |

**Ayrışma noktası.** Çin'de ölümsüzlük arar, Kore'de karaciğer yer, Japonya'da bazen koruyucudur.

### C · Gece cadısı — *The Night Hag* · `G264`

> Loğusayı ve yeni doğanı avlayan dişi varlık

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Lámia | Hellenic Ω | — |
| Lamashtu | Sumer 𒀭 | — |
| Strix | Romana SPQR | — |
| Àbíkú | Yorùbá · Ashanti ✺ | — |
| Adze | Yorùbá · Ashanti ✺ | — |
| Qalupalik | Inuit ᐃ | — |
| Al Karısı | Türk ☾ | — |
| Kenas-unarpe | Ainu ᚼ | — |
| Ma lai | Việt ☴ | — |
| Krasue | Siam ☸ | — |
| Aswang | Filipin ✧ | — |
| Manananggal | Filipin ✧ | — |
| Pontianak | Nusantara ❋ | — |
| Lilith | Talmud ✡ | — |

**Ayrışma noktası.** Dokuz gelenek, bir korku: doğum ölümlerinin folklora dönüşmesi.

### D · Fırtına kuşu — *The Storm Bird* · `B31`

> Kanadı gök gürültüsü, gözü şimşek olan dev kuş

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Bennu | Kemet 𓂀 | — |
| Garuḍa | Bharatiya ॐ | — |
| Anzû | Sumer 𒀭 | — |
| Sīmurgh | Pārs 𐎩 | — |
| Rukh | ʿArab ☪ | — |
| Khyung | Bod ☷ | — |
| Ziz | Talmud ✡ | — |
| Impundulu | Nguni ◈ | — |
| Animikii | Anishinaabe ▲ | — |

**Ayrışma noktası.** Mezopotamya'da hırsız, İran'da bilge, Kuzey Amerika'da savaşçı.

### E · Derinlerin yılanı — *The Serpent of the Deep* · `B11.2.1.1`

> Dünyayı çevreleyen veya dibinde yatan yılan

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Apep | Kemet 𓂀 | — |
| Jörmungandr | Norðr ᚦ | — |
| Yamata-no-Orochi | Yamato 神 | — |
| Nāga | Bharatiya ॐ | — |
| Imugi | Hangug 단 | — |
| Zmey | Slovjan ⚡ | — |
| Aži Dahāka | Pārs 𐎩 | — |
| Taniwha | Mā'ohi ᴥ | — |
| Iku-Turso | Suomi ᛉ | — |
| Thuồng luồng | Việt ☴ | — |
| Phaya Nak | Siam ☸ | — |
| Vishap | Hayk ✚ | — |
| Inkanyamba | Nguni ◈ | — |
| Zmeu | Dacia ✠ | — |
| Amaru | Tawantinsuyu ☉ | — |

**Ayrışma noktası.** Kuzeyde kıyameti getirir, Mısır'da her gece yenilir, And'da iki dünyayı bağlar.

### F · Eşik bekçisi — *The Threshold Guardian* · `F150`

> Geçilmesi gereken kapıda duran varlık

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Kérberos | Hellenic Ω | — |
| Ḫumbaba | Sumer 𒀭 | — |
| Basiliscus | Romana SPQR | — |
| Qílín | Zhōnghuá 龍 | — |
| Camazotz | Maya 𝋠 | — |
| Aralez | Hayk ✚ | — |
| Golem | Talmud ✡ | — |
| Temes Savsap | Melanesia ◉ | — |

**Ayrışma noktası.** Bazıları cezalandırır, bazıları yalnızca bakar — ve bakış yeterlidir.

### G · Yaban adamı — *The Wild Man* · `F460`

> Ormanda/dağda yaşayan, insana benzeyen ama insan olmayan

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Basajaun | Euskal ✜ | — |
| Migoi | Bod ☷ | — |
| Almas | Mongol ⚔ | — |
| Ochokochi | Kartveli ✛ | — |
| Curupira | Tupi-Guarani ❂ | — |
| Stállu | Sápmi ❄ | — |

**Ayrışma noktası.** Baskça'da çobanı korur, Amazon'da avcıyı cezalandırır — doğanın tarafı.

### H · Gizli halk — *The Hidden People* · `F251`

> Yanı başımızda ama görünmeyen bir topluluk

| Üye | Gelenek | Sayfa |
|---|---|---:|
| Koropokkuru | Ainu ᚼ | — |
| Orang Bunian | Nusantara ❋ | — |
| Ulda | Sápmi ❄ | — |
| Huldufólk | Ísland ❆ | — |

**Ayrışma noktası.** İzlanda'da hâlâ yol güzergâhı değiştirtir — yaşayan folklor.

## 4. Telaffuz rehberi

| Ad | Telaffuz | Sayfa |
|---|---|---:|
| **Àbíkú** | — | — |
| **Adaro** | ah-DAH-ro | — |
| **Adze** | — | — |
| **Ahuizotl** | — | — |
| **Ajatar** | — | — |
| **Al Karısı** | — | — |
| **Almas** | — | — |
| **Amarok** | — | — |
| **Amaru** | — | — |
| **Ammit** | — | — |
| **Anansi** | — | — |
| **Animikii** | — | — |
| **Anzû** | — | — |
| **Apep** | — | — |
| **Aralez** | ah-rah-LEZ | — |
| Arlez | *bkz.* Aralez | — |
| **Aswang** | — | — |
| ataro | *bkz.* Adaro | — |
| **Aži Dahāka** | — | — |
| **Basajaun** | — | — |
| **Basiliscus** | — | — |
| **Bean sídhe** | — | — |
| **Bennu** | — | — |
| **Boitatá** | — | — |
| **Buda** | — | — |
| **Camazotz** | — | — |
| **Chímaira** | — | — |
| **Chötgör** | — | — |
| **Cipactli** | — | — |
| **Curupira** | — | — |
| **Devi** | — | — |
| **Dokkaebi** | — | — |
| **Domovoy** | — | — |
| **Draugr** | — | — |
| **Each-uisce** | — | — |
| **Fenrir** | — | — |
| **Ganen** | — | — |
| **Garuḍa** | — | — |
| **Ghūl** | — | — |
| **Golem** | — | — |
| **Gufihtar** | — | — |
| **Herensuge** | — | — |
| **Huldufólk** | — | — |
| **Húli jīng** | — | — |
| **Ḫumbaba** | — | — |
| **Iara** | — | — |
| **Iku-Turso** | — | — |
| **Impundulu** | — | — |
| **Imugi** | — | — |
| **Inkanyamba** | — | — |
| **Jörmungandr** | — | — |
| **Kaji** | — | — |
| **Kappa** | — | — |
| **Karakoncolos** | — | — |
| **Kenas-unarpe** | KEH-nas oo-NAR-peh | — |
| Kenash-unarabe | *bkz.* Kenas-unarpe | — |
| **Kérberos** | — | — |
| **Khyung** | — | — |
| **Kinnarī** | — | — |
| Koro-pok-guru | *bkz.* Koropokkuru | — |
| **Koropokkuru** | ko-ro-POK-koo-roo | — |
| Korpokkur | *bkz.* Koropokkuru | — |
| **Krasue** | — | — |
| **Kumiho** | — | — |
| **Lamashtu** | — | — |
| **Lamia** | — | — |
| **Lámia** | — | — |
| **Lemures** | — | — |
| **Lilith** | — | — |
| **Lóng** | — | — |
| **Ma lai** | — | — |
| **Makara** | — | — |
| **Manananggal** | — | — |
| **Masalai** | mah-sah-LYE | — |
| masalai ples | *bkz.* Masalai | — |
| **Migoi** | — | — |
| **Mishipeshu** | — | — |
| **Moʻo** | — | — |
| **Nāga** | — | — |
| **Nahual** | — | — |
| **Näkki** | — | — |
| **Nhang** | n-HAHNG | — |
| Nhang (deniz canavarı, genel) | *bkz.* Nhang | — |
| Nitat-unarabe | *bkz.* Kenas-unarpe | — |
| **Nykur** | — | — |
| **Ochokochi** | — | — |
| **Olgoi-Khorkhoi** | — | — |
| **Ông Ba Mươi** | — | — |
| **Orang Bunian** | — | — |
| **Perī** | — | — |
| **Phaya Nak** | — | — |
| **Pishtaco** | — | — |
| **Ponaturi** | — | — |
| **Pontianak** | — | — |
| **Púca** | — | — |
| **Qalupalik** | — | — |
| **Qílín** | — | — |
| **Rangda** | — | — |
| **Repun Kamuy** | REH-pun KAH-mooy | — |
| Repun-riri-kata inao uk kamui | *bkz.* Repun Kamuy | — |
| **Rukh** | — | — |
| **Rusalka** | — | — |
| **Şahmeran** | — | — |
| **Sīmurgh** | — | — |
| **Skuggabaldur** | — | — |
| **Srin-po** | — | — |
| **Stállu** | — | — |
| **Strigoi** | — | — |
| **Strix** | — | — |
| **Supay** | — | — |
| **Taniwha** | — | — |
| **Temes Savsap** | TEH-mes SAV-sap | — |
| **Tengu** | — | — |
| **Thuồng luồng** | — | — |
| **Tikbalang** | — | — |
| **Tokoloshe** | — | — |
| **Tupilaq** | — | — |
| **Ulda** | — | — |
| **Vârcolac** | — | — |
| višap | *bkz.* Vishap | — |
| **Vishap** | vee-SHAHP | — |
| vishapakar (dikit) | *bkz.* Vishap | — |
| **Way** | — | — |
| **Windigo** | — | — |
| **Xtabay** | — | — |
| **Yamata-no-Orochi** | — | — |
| Yaralez | *bkz.* Aralez | — |
| **Zar** | — | — |
| **Ziz** | — | — |
| **Zmeu** | — | — |
| **Zmey** | — | — |
| **ʿIfrīt** | — | — |

